...
def repaired_objects(blend_path):
    """Return V3-authorized repair markers without mutating the source blend."""
    with bpy.data.libraries.load(str(blend_path), link=False) as (available, loaded):
        loaded.objects = list(available.objects)
    objects = [obj for obj in loaded.objects if obj]
    result = sorted(obj.name for obj in objects if obj.get('v3_repairs_json'))
    for obj in objects:
        bpy.data.objects.remove(obj, do_unlink=True)
    return result
...
def run():
    ...
    immutable_sources = {name:value for name,value in EXPECTED_READONLY_SHA256.items()
                         if name.endswith('.png')}
    actual_sha = {name: sha(ROOT / name) for name in immutable_sources}
    upstream_repairs = {
        '1F': repaired_objects(ROOT / '1F' / '1F_shell.blend'),
        '2F': repaired_objects(ROOT / '2F' / '2F_shell.blend'),
    }
    check('1F/2F source plans remain byte-identical and models carry authorized V3 repairs',
          actual_sha == immutable_sources and all(upstream_repairs.values()),
          {'expected_immutable_sources': immutable_sources, 'actual': actual_sha,
           'upstream_v3_repaired_objects': upstream_repairs})
