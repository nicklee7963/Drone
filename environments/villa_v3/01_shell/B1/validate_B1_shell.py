...
def repaired_objects(path):
    """Read V3 repair markers from another floor without modifying its blend."""
...
def run():
    ...
    repair_markers = {path.relative_to(ROOT).as_posix(): repaired_objects(path) for path in OTHER_FLOORS}
    authorized = bool(baseline) and all(
        current.get(name) == expected or bool(repair_markers.get(name))
        for name, expected in baseline.items()
    )
    check("Upper-floor shells are unchanged or carry authorized V3 repairs", authorized,
          {"baseline": baseline, "current": current, "v3_repaired_objects": repair_markers})
