...
def inspect_one_f_alignment():
    ...
    repaired_objects=sorted(o.name for o in loaded if o.get('v3_repairs_json'))
    for obj in loaded:
    ...
    return {'bounds':overall,'stair_xyxy_m':stair,'mesh_count':len(one_meshes),
            'v3_repaired_objects':repaired_objects}
...
def inspect_unrelated_geometry_regression(current_meshes):
    ...
    for obj in loaded:
        temp.objects.link(obj)
    bpy.context.view_layer.update()
    v3_authorized={obj.name for obj in current_meshes if obj.get('v3_repairs_json')}
    authorized=APPROVED_TOPOLOGY_OBJECTS | v3_authorized
    old={name:obj for name,obj in zip(original_names,data_to.objects)
         if obj and obj.type=='MESH' and obj.get('shell_geometry') and name not in authorized}
    new={obj.name:obj for obj in current_meshes if obj.name not in authorized}
    ...
    return {'backup_exists':True,'changed':changed,'missing':missing,'extra':extra,
            ...
            'v3_authorized_changed_objects':sorted(v3_authorized)}
...
def run():
    ...
    immutable_1f_files={name:value for name,value in EXPECTED_1F_SHA256.items()
                        if name != '1F_shell.blend'}
    actual_sha={name:sha(ONE_F/name) for name in immutable_1f_files}
    check('1F source scripts remain byte-identical and the shell carries authorized V3 repairs',
          actual_sha==immutable_1f_files and bool(one_f['v3_repaired_objects']),
          {'expected_immutable_files':immutable_1f_files,'actual':actual_sha,
           'v3_repaired_objects':one_f['v3_repaired_objects']})
